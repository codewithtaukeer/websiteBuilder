...
import { ArrowUpRightFromSquareIcon, Bot, BotIcon, EyeIcon, Loader2Icon, SendIcon, UserIcon } from 'lucide-react';
      ...
      <div className='flex flex-col h-full'>
                ...
                if(isMessage){
                    ...
                    return(
                        <div key={msg.id} className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <div className={`rounded-2xl p-2 px-4 shadow-sm text-sm mt-5 max-w-[80%] leading-relaxed ${isUser ? 'bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none' : 'text-gray-100 bg-gray-800 rounded-tl-none'}`}>
                                {msg.content}
                            </div>
                    ...
                    return(
                        <div key={ver.id} className='w-4/5 mx-auto my-2 rounded-xl flex flex-col shadow gap-2  bg-gray-800 text-gray-200 '>
                            <div className='text-xs font-medium '>
                                code updated <br/>
                                <span className='text-gray-500 text-xs font-normal'>
                                    {new Date(ver.timestamp).toLocaleDateString()}</span>
                                
                            </div>
                            <div className='flex items-center justify-between'>
                                {
                                    project.current_version_index===ver.id?(<button className='px-3 py-1 rounded-md text-xs bg-gray-700'>
        ...
        <form onSubmit={handleRevisions} className='m-3 relative'>
            <div className='flex items-center gap-2'>
                ...
                className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring 
                ring-gray-700 focus:rising-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400
                transition-all' disabled={isGenerating}
                />
                ...
                transition-colors disabled:opacity-60'> 
                    {
                        isGenerating ? <Loader2Icon className='size-7 p-1.5 text-white animate-spin'/> : <SendIcon className='size-7 p-1.5 text-white'/>
                    }
                </button>
            </div>
        </form>
