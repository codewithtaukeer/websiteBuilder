...
import { ArrowBigDownDashIcon, EyeIcon, EyeOffIcon, FullscreenIcon, Laptop2Icon, LaptopIcon, Loader2Icon, MessageSquareIcon, SaveIcon, SmartphoneIcon, TabletIcon, XIcon } from 'lucide-react'
...
const fetchProject = async () => {
    ...
    setTimeout(()=>{
        if(project) setProject({...project,conversation:dummyConversations,versions:dummyVersion})
        setLoading(false)
...
sm:text-sm'>
  <button disabled={isSaving} onClick={saveProject} className='max-sm:hidden bg-gray-800 hover:bg-gray-700 
  text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors border border-gray-700
'>
    <SaveIcon size={16}/> Save
  </button>
  <Link className='flex items-center gap-2 px-4 py-1 rounded sm:rounded-sm border 
